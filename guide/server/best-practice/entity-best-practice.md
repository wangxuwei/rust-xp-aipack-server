...
These are the best practices for entities to match database
...
- should have a entity for global "Org", and for Update with "OrgForUpdate", for create it is "OrgForCreate"
- and there is a filter struct  "OrgFilter" for query results by filter
- then have "OrgBmc" which is crud for the entity, which generated by "generate_common_bmc_fns"
- then we have basics tests for the entities
- if a field is enum, then we create an enum like "OrgKind", and implement for From and nullable trait
